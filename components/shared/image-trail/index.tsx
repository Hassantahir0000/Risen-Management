...
class ImageTrailVariant9 {
  ...
  private render() {
    ...
    const deltaTime = Math.max(currentTime - this.lastFrameTime, 16) / 1000; // Convert to seconds, min 16ms
    this.lastFrameTime = currentTime;

    const distance = getMouseDistance(this.mousePos, this.lastMousePos);
    this.currentSpeed = distance / deltaTime; // pixels per second
    
    // Adjust lerp speed based on cursor speed - faster movement = less smoothing
    const lerpSpeed = Math.min(0.3, Math.max(0.05, this.currentSpeed / 1000));
    this.cacheMousePos.x = lerp(this.cacheMousePos.x, this.mousePos.x, lerpSpeed);
    this.cacheMousePos.y = lerp(this.cacheMousePos.y, this.mousePos.y, lerpSpeed);

    // Dynamic threshold - lower threshold when moving fast
    const dynamicThreshold = Math.max(5, this.threshold - (this.currentSpeed / 50));

    if (distance > dynamicThreshold && this.canShowNext && this.isIdle) {
      this.showNextImage();
      this.lastMousePos = { ...this.mousePos };
    }
    if (this.isIdle && this.zIndexVal !== 1) {
  ...
  private showNextImage() {
    ...
    // Calculate duration based on speed - faster movement = shorter duration
    // Speed range: 0-2000 px/s maps to duration: 1.2s - 0.15s
    const maxSpeed = 2000;
    const minDuration = 0.15;
    const maxDuration = 1.2;
    const normalizedSpeed = Math.min(this.currentSpeed / maxSpeed, 1);
    const duration = maxDuration - (normalizedSpeed * (maxDuration - minDuration));

    // Use current mouse position when moving fast, cached position when slow
    const useCurrentPos = this.currentSpeed > 300;
    const startX = useCurrentPos 
      ? this.mousePos.x - (img.rect?.width ?? 0) / 2
      : this.cacheMousePos.x - (img.rect?.width ?? 0) / 2;
    const startY = useCurrentPos
      ? this.mousePos.y - (img.rect?.height ?? 0) / 2
      : this.cacheMousePos.y - (img.rect?.height ?? 0) / 2;
    const endX = this.mousePos.x - (img.rect?.width ?? 0) / 2;
    const endY = this.mousePos.y - (img.rect?.height ?? 0) / 2;
